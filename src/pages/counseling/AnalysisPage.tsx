...
const SpeechDetailPage = () => {
  ...
  const audioRef = useRef<HTMLAudioElement | null>(null); // 오디오 객체 관리
  ...
  const [recordingDbList, setRecordingDbList] = useState<number[]>([]);
  ...
  const streamRef = useRef<MediaStream | null>(null); // 스트림을 미리 로드하여 보관
  const isWaitingForVoiceRef = useRef(false); // interval 내에서 최신 상태 참조용
  const audioChunksRef = useRef<Blob[]>([]);

  const audioContextRef = useRef<AudioContext | null>(null);
  const [selectedVersions, setSelectedVersions] = useState<any>({}); // 각 세그먼트별 선택된 버전 관리 {segmentId: versionIndex}
        ...
        intervalRef.current = setInterval(() => {
            ...
            if (isWaitingForVoiceRef.current) {
                ...
                if (db > -30) {
                    ...
                    isWaitingForVoiceRef.current = false;
                    // 첫 데이터 포인트 추가
                    setRecordingDbList((prev: number[]) => [...prev, db]);
                }
    ...
    setTimeout(async () => {
        ...
        if (currentSegment?.segment_id) {
            ...
            try {
                ...
                queryClient.setQueryData(["speech", speechId], (oldData: any) => {
                    if (!oldData || !oldData.scripts) return oldData;

                    const newScripts = oldData.scripts.map((script: any) => ({
                        ...script,
                        segments: script.segments.map((seg: any) => {
  ...
  const isAnyReRecordingSelected = useMemo(() => {
    return allSegments.some((seg: any) => {
        // 1. 현재 선택된 버전 인덱스 확인
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        const selectionsPayload = {
            selections: allSegments.map((seg: any) => {
                // 1. 현재 선택된 버전 인덱스 확인
                let selectedIdx = selectedVersions[seg.segment_id];
  ...
  const chartData = {
    datasets: [
      {
        label: 'Original dB',
        data: dBList.map((val: any, i: number) => ({ x: i * originalInterval, y: val })), // 0.1초 고정 간격
      ...
      {
        label: 'Recording dB',
        data: (isRecording ? recordingDbList : displayRedDbList).map((val: any, i: number) => ({ x: i * 0.1, y: val })), 
  ...
  const getAudioUrlForSegment = (segment: any, isCurrentSegment = false) => {
